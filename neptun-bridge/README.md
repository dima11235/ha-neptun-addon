...
Локальный MQTT‑мост и парсер бинарных кадров Neptun AquaControl. Аддон работает рядом с Home Assistant: принимает кадры из «облака» Neptun, проверяет целостность, раскладывает данные по понятным MQTT‑топикам и автоматически объявляет сущности в HA (MQTT Discovery). Команды из HA отправляются обратно на устройство через облачный MQTT‑канал.
